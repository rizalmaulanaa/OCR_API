...
def preprocessing(file_path, inverted=True, normalize=True, 
                  smoothing=False, sm_kernel=(11,11), save_img=True):

    img_list, output_path = [],[]
    for path in file_path:
        # Used the read image function
        img_temp = read_image(path)
        
        # Used the inverted function
        if inverted:
            img_temp = inverted_image(img_temp)

        # Used the normalize function
        if normalize:
            img_temp = normalize_image(img_temp)

        # Smoothing operation
        if smoothing:
            img_temp = cv.GaussianBlur(img_temp,sm_kernel,0)
        img_list.append(img_temp)

        # Get new image path
        if inverted:
            folder_new = config_dict['path_saved'] + 'data_preprocessing_inverted_temp/'
        else:
            folder_new = config_dict['path_saved'] + 'data_preprocessing/'

        img_name = path.split('/')[-1]
        path_new = folder_new + img_name
        output_path.append(path_new)
